...
static void Check(bool value) { if(!value) throw new Exception("Native binding contract failed."); }
...
var path=Path.Combine(Path.GetTempPath(),"EWAF-"+Guid.NewGuid()); Directory.CreateDirectory(path);
try {
    ...
    var contents=Path.Combine(path,"09-03-2026","keep.txt"); File.WriteAllText(contents,"Keep my work");
    ...
    result=await Core.Create(plan,path,token.Token,new Progress<JsonElement>()); Check(result.GetProperty("cancelled").GetBoolean());
