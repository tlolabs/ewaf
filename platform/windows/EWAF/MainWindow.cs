...
public sealed class MainWindow : Window {
    ...
    private readonly TextBlock status = new() { Text = "Choose a date range and weekday, then review your folders.", TextWrapping = TextWrapping.Wrap, IsTextSelectionEnabled = true };
    private readonly TextBlock destinationLabel = new() { Text = "Choose an existing destination folder.", TextWrapping = TextWrapping.Wrap, IsTextSelectionEnabled = true };
    ...
    public MainWindow() {
        ...
        AddMenu(file,"New Window",App.NewWindow); AddMenu(file,"Choose Destination…",async () => await Choose());
        AddMenu(file,"Create Folders",async () => await Create()); AddMenu(file,"Cancel Folder Creation",() => operation?.Cancel());
        AddMenu(file,"Settings",async () => await Settings()); AddMenu(file,"Close",Close);
        var help = new MenuBarItem { Title = "Help" };
        AddMenu(help,"EWAF Help",async () => await Launcher.LaunchUriAsync(new Uri("https://github.com/tlolabs/ewaf#use")));
        AddMenu(help,"Download Updates…",async () => await Launcher.LaunchUriAsync(new Uri("https://github.com/tlolabs/ewaf/releases")));
        AddMenu(help,"About EWAF",async () => await Message("EWAF", "Every Week a Folder\nVersion " + Core.Call(new {op="info"}).GetProperty("version").GetString()));
        menu.Items.Add(file); menu.Items.Add(help); Root.Children.Add(menu);
        var body = new Grid { Padding = new Thickness(20), ColumnSpacing = 24 };
        body.ColumnDefinitions.Add(new ColumnDefinition { Width = new GridLength(310) }); body.ColumnDefinitions.Add(new ColumnDefinition());
        Grid.SetRow(body,1); Root.Children.Add(body);
        form.Children.Add(new TextBlock { Text = "Every Week a Folder", Style = (Style)Application.Current.Resources["TitleTextBlockStyle"] });
        form.Children.Add(start); form.Children.Add(CalendarButton(start,"Choose start date")); form.Children.Add(end); form.Children.Add(CalendarButton(end,"Choose end date"));
        foreach (uint d in Days) weekday.Items.Add(CultureInfo.CurrentCulture.DateTimeFormat.DayNames[d-1]);
        form.Children.Add(weekday);
        form.Children.Add(new TextBlock { Text = "Both dates are included. Exact entry supports years 0001–9999.", TextWrapping = TextWrapping.Wrap });
        form.Children.Add(destinationLabel); form.Children.Add(choose); form.Children.Add(open);
        body.Children.Add(new ScrollViewer { Content = form, HorizontalScrollBarVisibility = ScrollBarVisibility.Disabled });
        var right = new Grid { RowSpacing = 10 };
        right.RowDefinitions.Add(new RowDefinition { Height = GridLength.Auto }); right.RowDefinitions.Add(new RowDefinition { Height = GridLength.Auto }); right.RowDefinitions.Add(new RowDefinition()); right.RowDefinitions.Add(new RowDefinition { Height = GridLength.Auto });
        right.Children.Add(count); Grid.SetRow(search,1); right.Children.Add(search); Grid.SetRow(preview,2); right.Children.Add(preview);
        var note = new TextBlock { Text = "Preview shows up to 200 matches in date order. Existing folders will be kept.", TextWrapping = TextWrapping.Wrap }; Grid.SetRow(note,3); right.Children.Add(note);
        Grid.SetColumn(right,1); body.Children.Add(right);
        var footer = new StackPanel { Spacing = 8, Padding = new Thickness(20) };
        var actions = new StackPanel { Orientation = Orientation.Horizontal, Spacing = 12 }; actions.Children.Add(create); actions.Children.Add(cancel);
        ...
        weekday.SelectedIndex = Array.IndexOf(Days,saved); if (weekday.SelectedIndex < 0) weekday.SelectedIndex = 3;
        start.TextChanged += (_,_) => Refresh(); end.TextChanged += (_,_) => Refresh(); weekday.SelectionChanged += (_,_) => Refresh(); search.TextChanged += (_,_) => Refresh();
        choose.Click += async (_,_) => await Choose(); create.Click += async (_,_) => await Create(); cancel.Click += (_,_) => operation?.Cancel(); open.Click += async (_,_) => { if (destination != null) await Launcher.LaunchFolderAsync(await Windows.Storage.StorageFolder.GetFolderFromPathAsync(destination)); };
    ...
    private CalendarDatePicker CalendarButton(TextBox target,string label) {
        var picker = new CalendarDatePicker { PlaceholderText=label, MinDate=new DateTimeOffset(1600,1,1,0,0,0,TimeSpan.Zero), MaxDate=new DateTimeOffset(9999,12,31,0,0,0,TimeSpan.Zero) };
        AutomationProperties.SetName(picker,label);
        picker.DateChanged += (_,_) => { if (picker.Date is { } date) target.Text=date.ToString("MM-dd-yyyy",CultureInfo.InvariantCulture); };
        return picker;
    }
    private void Copy() { if(preview.SelectedItem is string name) { var data=new DataPackage(); data.SetText(name); Clipboard.SetContent(data); } }
    private async void Refresh() {
        if (busy || weekday.SelectedIndex < 0) return;
        var current = ++generation; plan=null; create.IsEnabled=false; preview.ItemsSource=null;
        var first=start.Text; var last=end.Text; var day=Days[weekday.SelectedIndex]; var query=search.Text;
        try {
            ...
            plan=result.input; total=result.Item2.GetProperty("count").GetInt32(); confirmation=result.Item2.GetProperty("requiresConfirmation").GetBoolean();
            var rows=result.Item2.GetProperty("dates").EnumerateArray().Select(v=>v.GetString()!).ToList(); preview.ItemsSource=rows;
            count.Text=$"{total:N0} folders"; create.IsEnabled=total>0;
            status.Text=total==0 ? "No matching dates. Choose a wider range or another weekday." : rows.Count==0 ? "No search matches. Creation still uses the full date range." : "Review your folders, then choose Create Folders.";
        } catch(Exception e) { if(current==generation && !closed) { count.Text="Check Your Dates"; status.Text=e.Message; } }
    }
    private async Task<bool> Choose() {
        if(busy || dialogOpen) return false;
        dialogOpen=true;
        try {
            var picker=new FolderPicker(); picker.FileTypeFilter.Add("*");
            WinRT.Interop.InitializeWithWindow.Initialize(picker,WinRT.Interop.WindowNative.GetWindowHandle(this));
            var folder=await picker.PickSingleFolderAsync(); if(folder==null || closed) return false;
            destination=folder.Path; destinationLabel.Text=destination; open.IsEnabled=true; return true;
        } catch(Exception e) { status.Text="The folder could not be opened. " + e.Message; return false; }
        finally { dialogOpen=false; }
    }
    private async Task Create() {
        if(busy || dialogOpen || plan==null || total==0) return;
    ...
    private async Task<bool> Confirm() {
        dialogOpen=true;
        try { return await new ContentDialog { XamlRoot=Root.XamlRoot,Title=$"Create {total:N0} folders?",Content="This is a large operation. Existing folders and their contents will be preserved. You can cancel while it runs.",PrimaryButtonText="Continue",CloseButtonText="Cancel",DefaultButton=ContentDialogButton.Close }.ShowAsync()==ContentDialogResult.Primary; }
        finally { dialogOpen=false; }
    }
    private async Task Message(string title,string text) {
        if(dialogOpen || closed) return; dialogOpen=true;
        try { await new ContentDialog { XamlRoot=Root.XamlRoot,Title=title,Content=text,CloseButtonText="OK" }.ShowAsync(); }
        finally { dialogOpen=false; }
    }
    private async Task Settings() {
        ...
        foreach(uint day in Days) choice.Items.Add(CultureInfo.CurrentCulture.DateTimeFormat.DayNames[day-1]);
        uint.TryParse(Preferences.Read("defaultWeekday","5"),out var saved); choice.SelectedIndex=Array.IndexOf(Days,saved); if(choice.SelectedIndex<0) choice.SelectedIndex=3;
        AutomationProperties.SetName(choice,"Default weekday"); dialogOpen=true;
        try { if(await new ContentDialog { XamlRoot=Root.XamlRoot,Title="Settings",Content=choice,PrimaryButtonText="Save",CloseButtonText="Cancel" }.ShowAsync()==ContentDialogResult.Primary) Preferences.Write("defaultWeekday",Days[choice.SelectedIndex].ToString()); }
        catch(Exception e) { status.Text="Settings could not be saved. " + e.Message; }
        finally { dialogOpen=false; }
    }
}
