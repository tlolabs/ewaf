...
WinUI schedules Rust on Task.Run, marshals progress to the UI context, and registers cancellation independently of worker execution. Its unpackaged per-user settings use HKCU\Software\tlolabs\EWAF; ApplicationData.Current requires a packaged identity and is not used. FolderPicker is initialized with the native window handle. The publish target explicitly stages PRI/XBF resources; startup resolves resources from the application directory regardless of launch working directory. Tests isolate preferences under a GUID-specific test registry key. GTK uses GTask, retained window/application references and GSettings. It keeps the app alive until a closing window's operation has canceled and released its directory handle. Settings belong to the native layers, not Rust.
