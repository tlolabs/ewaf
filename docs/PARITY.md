...
- Windows: native compilation, packaged C# tests and WinUI accessible-control/date-validation checks pass on both x64 and ARM64. Explicit PRI/XBF staging resolves the packaged startup resource failure. The added installation/confirmation checks are required CI gates; consult the latest run for completion.
