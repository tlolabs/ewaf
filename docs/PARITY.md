...
| --- | --- | --- | --- |
| Date selection, weekday, exact/historical dates | Existing UI + 31 Swift regressions pass through Rust | Native x64/ARM64 builds and packaged date-validation UI test | GTK widget validation passes on Ubuntu amd64/arm64 |
...
| Full-range arithmetic, naming, thresholds | Shared Rust fixtures/tests pass | Native Rust tests pass on x64/ARM64 | Same core |
...
- Windows: native compilation, packaged C# tests and WinUI accessible-control/date-validation checks pass on both x64 and ARM64. Explicit PRI/XBF staging resolves the packaged startup resource failure. Large-operation confirmation and per-user installation/removal tests pass on x64; the same checks are required on ARM64. Consult the latest run for completion.
...
Runner evidence: [macOS and Linux verification](https://github.com/tlolabs/ewaf/actions/runs/35064152913), [packaged Windows verification](https://github.com/tlolabs/ewaf/actions/runs/35065461278). Every final release still requires the complete matrix to pass on its own commit.
