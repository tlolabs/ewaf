...
Icon validation checks PNG integrity, ICO/ICNS sizes, artwork hashes and matching Composer layer geometry. macOS builds require Xcode 26+; CI uses Xcode 26.3 on both architectures, running Apple Silicon on macOS 26 and Intel on macOS 15. The system assetutil on macOS 15 checks compatibility images; a required macOS 26 job revalidates both Mac archives and their native stacks before release. Package validation uses assetutil to require native light, dark and tinted EWAF icon stacks, all three vector layers, and an ICNS compatibility icon. It also requires both embedded executable and window icons on Windows, and the application-named SVG/desktop entry on Linux. GTK widget tests also resolve the embedded icon without a system installation.

For local icon changes, use `script/build_and_run.sh`: it updates the bundle modification date and force-refreshes that bundle's Launch Services registration before opening it. This prevents cached Dock artwork from surviving an in-place development rebuild with the same version. Icon checks also enforce the Sunday-first calendar with Tuesday and Thursday selected.
